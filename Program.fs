...
let generateInstanceAndSolution n =
  ...
  let solution: Solution = Algorithms.Reference.run instance

  let inFilename = "in" + (string indexNumber) + "_" + (string n) + ".txt"
  let outFilename = "out" + (string indexNumber) + "_" + (string n) + ".txt"

  instance |> Serialization.Instance.serialize |> writeToFile inFilename
  solution |> Serialization.Solution.serialize |> writeToFile outFilename
...
let solveAllInDirectory solver dirName =
  ...
  let instances = Array.map Serialization.Instance.fromFile filenames
...
let measureSolving (path: string) =
  ...
  let instance = Serialization.Instance.fromFile path

  let stopWatch = Diagnostics.Stopwatch.StartNew()
  let solution = Algorithms.List.run instance
  stopWatch.Stop()

  solution
  |> Serialization.Solution.serialize
